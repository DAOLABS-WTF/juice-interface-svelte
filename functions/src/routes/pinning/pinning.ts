...
import { uploadFiles, uploadJsonEntries } from '../../utils/ipfs';
...
function handleEntry(entry: any) {
	...
	for (const key in entry) {
		...
		} else if (typeof entry[key] === 'string' && entry[key]?.toLowerCase() === 'true') {
			obj[key] = true;
		} else if (typeof entry[key] === 'string' && entry[key]?.toLowerCase() === 'false') {
			obj[key] = false;
		} else {
			let res = entry[key];
			try {
...
export const pinningFunc = async (request: functions.Request, response: functions.Response) => {
		...
		try {
			...
			for (let i = 0; i < entries.length; i++) {
				for (let j = 0; j < entries[i].attributes[1].value; j++) {
					...
					const file = new File([compressedContent], `compressed/${tokenCount}`);
...
export async function pinFiles(request: Request & { files }, response: Response) {
	try {
		const contentType = request.headers['content-type'];

		let filename = `file-${Date.now()}`;
		let content: any;
		const [firstFile] = (request.files || []) as {
			fieldname: 'file';
		...
		if (contentType.match(/application\/json/i)) {
			if (request.body.content) {
				...
				content = JSON.stringify(request.body, null, '  ');
			}
		} else if (firstFile) {
