...
function getRandomFadeAnimation(index: number) {
	const randomSeconds = Math.random() + 2 * 15;
	const classValue = `animation: fade ${randomSeconds}s ease-in-out ${Math.random() *
		20}s infinite`;
	return `.piece${index}{${classValue}}`;
}

function getRandomSpinAndFadeAnimation(index: number, matrix: string) {
	const randomSeconds = Math.random() + 2 * 15;
	const randomSpinSeconds = Math.random() * 2 + 5;

	let styles = '';

	const classValue = `animation: spin${index} ${randomSpinSeconds}s ease-in-out ${Math.random() *
	...
	const keyframes = `
			@keyframes spin${index} {
				0% {
					transform: ${matrix} rotate(${reverse ? 360 : 0}deg);
				}
				100% {
					transform: ${matrix} rotate(${reverse ? 0 : 360}deg);
				}
			}
		`;

	styles += keyframes;
	styles += `.piece${index}{${classValue}}`;
	return styles;
}

export function generateTile(address: string, animate = false) {
	let str = head;
	// Animate specific variables
	let styles = ``;
	let nSpinPieces = [];
	...
	if (animate) {
		...
		nSpinPieces = Array.from({ length: 7 }, () => Math.floor(Math.random() * 29));
		styles = `
		@keyframes fade {
			0% {
				opacity: 1;
	...
	for (let r = 0; r < 3; r++) {
		for (let i = 0; i < 9; i++) {
			...
			const matrix = `matrix(1,0,0,1,${(i % 3) * 100},${Math.floor((i % 9) / 3) * 100})`;
			let tile = '';
			let pieceIndex = r * 9 + i;

			if (animate) {
				if (nSpinPieces.includes(pieceIndex)) {
					// Get the spin & fade style for this piece
					styles += getRandomSpinAndFadeAnimation(pieceIndex, matrix);
				} else {
					// Get the fade style for this piece
					styles += getRandomFadeAnimation(pieceIndex);
				}
			}

			if (svg.startsWith('<path')) {
				tile = `
      <g transform="${matrix}" class="piece${pieceIndex}">
        ${svg
    ...
    `;
		...
		for (const ring of rings.filter(ring => ring.layer === r)) {
			let i,
				...
				posX = 0,
				posY = 0;
			const matrix = `matrix(1,0,0,1,${posX},${posY})`;
			switch (ring.size) {
				case 0:
