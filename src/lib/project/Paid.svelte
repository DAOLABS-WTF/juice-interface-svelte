<script lang="ts">
	...
	export let loadingVolume = false;
	export let loadingInTreasury = false;
	export let loadingDistributed = false;
	...
	<div class="stats">
		<InfoSpaceBetween>
			<div slot="left">
				...
				<Popover
					placement="right"
					message="The total funds this Juicebox project has received since it was created."
				>
			...
			<div slot="right">
				...
				<span class="amount" style="display: flex; gap: 0.5rem;">
					<Skeleton loading={loadingVolume} width="5rem" height="1rem" type="text">
						<ETHAmount
		...
		<div class="">
			<InfoSpaceBetween>
				...
				<div slot="right">
					<div class="amount">
						<Skeleton loading={loadingInTreasury} width="5rem" height="1rem" type="text">
							<span class="amount-main" style="display: flex; gap: 0.5rem;">
								{#if [V2_CURRENCY_ETH, NO_CURRENCY].includes(BigNumber.from($projectsContext?.distributionLimitCurrency || V2_CURRENCY_ETH).toNumber())}
									<ETHAmount amount={balanceInCurrency.div($ethPrice)} precision={2} />
								{:else}
            ...
            the next cycle begins."
				...
				<div slot="right">
					{#if loadingDistributed}
						<Skeleton loading={loadingDistributed} width="5rem" height="1rem" type="text" />
					{:else}
						<Popover
							slot="right"
						...
						>
							{#if distributionLimitType === DistributionLimitType.Specific}
								{#if [V2_CURRENCY_ETH, NO_CURRENCY].includes(BigNumber.from($projectsContext?.distributionLimitCurrency).toNumber())}
									<ETHAmount amount={$projectsContext.usedDistributionLimit} precision={2} />
									/
									<ETHAmount amount={$projectsContext.distributionLimit} precision={2} />
								{:else}
			...
			{#if loadingDistributed}
				<Skeleton loading={loadingDistributed} width="100%" height="0.5rem" type="text" />
			{:else if $projectsContext.distributionLimit}
				<FundingProgressBar
		...
		<InfoSpaceBetween>
			...
			<div slot="right">
				{#await ownerBalance}
					<Skeleton loading={true} width="5rem" height="1rem" type="text" />
				{:then amount}
					<Skeleton loading={!owner} width="5rem" height="1rem" type="text">
