...
<script lang="ts">
	...
	export let name: keyof typeof svgs | 'metamask' | 'sushiswap' | 'instagramColor' = 'caret';
	...
	let lazyLoaders = ['instagramColor', 'metamask', 'sushiswap'];
	let icon: SvelteComponent = undefined;

	async function load(name: string) {
		const module = await import(`./Icon${capitalize(name)}.svelte`);
		icon = module.default;
	}
